...
The `cert-manager` branch (this one) and the `mtls` branch look almost
identical — same cert-manager, same JKS keystore, same Skaffold flow, HTTPS on
8443 — yet they answer two different questions, and the difference is easy to
miss:
...
- **`mtls` — mutual TLS.** Everything above, **plus** the server *requires and
  verifies a client certificate* (`server.ssl.client-auth = need`). The exact
  same command above is now **rejected** during the handshake:
...
single self-signed issuer and never asks the client for anything. See the
[Mutual TLS (mTLS)](#mutual-tls-mtls) section on the `mtls` branch for the full
setup.
